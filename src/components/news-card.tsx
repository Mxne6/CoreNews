...
import { getHeatLevel } from "@/lib/ui/categories";
...
const HEAT_TEXT_CLASS = "text-[#ff4d4f]";
...
export function NewsCard({ id, title, summaryCn, hotScore, index, meta, updatedAt }: NewsCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-2">
        ...
        <span className="shrink-0 rounded-md border border-white/12 bg-white/[0.04] px-2 py-0.5 text-[0.72rem] font-medium text-slate-200">
          {categoryToken}
        </span>

        <div className="min-w-0 flex flex-1 items-center gap-3">
          <h3 className="min-w-0 flex-1 text-[1.08rem] font-semibold leading-6 text-slate-100 sm:text-[1.16rem]">
            <Link
              href={`/news/${id}`}
              prefetch={false}
        ...
        <div className="w-[78px] shrink-0 text-right [font-variant-numeric:tabular-nums]">
          <span className={`inline-flex items-center justify-end gap-1 text-[0.98rem] font-semibold ${HEAT_TEXT_CLASS}`}>
            <span>{typeof hotScore === "number" ? hotScore.toFixed(1) : "--"}</span>
            <svg
              viewBox="0 0 24 24"
              className={`h-5 w-5 drop-shadow-[0_0_10px_currentColor] ${HEAT_TEXT_CLASS}`}
              fill="currentColor"
              aria-hidden
            >
